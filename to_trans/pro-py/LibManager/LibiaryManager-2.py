    ...
    def card_info(self):
        while(True):
            ...
            if card_choice == 1:
                ...
                self.cursor.execute(u"SELECT BookID FROM borrows WHERE ReaderID = %d" % card_number)
                book_bor = self.cursor.fetchall()
                for everybook in book_bor:
                    book_no = everybook[0]
                    self.cursor.execute(u"UPDATE books SET BookState = BookState + 1 WHERE BookID = %d" % book_no)
